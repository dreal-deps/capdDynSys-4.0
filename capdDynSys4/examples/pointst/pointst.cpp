...
void C0Test() {
  double grid = 30;
  int order = 20;
  IMap iVectorField = "par:c;var:x,y,z;fun:y,z,c^2-y-0.5*x^2;";
  ...
  DMap dVectorField = "par:c;var:x,y,z;fun:y,z,c^2-y-0.5*x^2;";
...
void C1Test() {
  ...
  IAffineSection s(IVector(3),IVector(0.,0.,1.));
  IOdeSolver solver(f, order);
  IPoincareMap pm(solver, s);
...
void C2Test() {
  ...
  int order = 30;
  IMap f = "var:x,y,z;fun:y,z,1-y-0.5*x^2;";
  INonlinearSection s = "var:x,y,z;fun:z;";
  IC2OdeSolver solver(f, order);
  IC2PoincareMap PM(solver, s);

  IMatrix DQ, DP1, DP2;
  IC2OdeSolver::HessianType D2P1(3), D2P2(3);
