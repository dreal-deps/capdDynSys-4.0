...
void verifyExistenceOfPeriodicOrbit(IPoincareMap& pm, IVector X, int period)
{
  ...
  // Compute iteration of Poincare map (3-dim).
  IVector y = pm(s1,returnTime,period);
  
  // Project it onto 2-dim Poicnare section.
  IVector imCenter(2,y.begin()+1);

  
  // Derivative of PM on the set X.
  // Define doubleton representation of for the first order variational equations.
...
// fixed point
    ...
    verifyExistenceOfPeriodicOrbit(pm,center+X,1);

// period 2 orbit
    center[0] = -5.4240738226652;
    center[1] = 0.031081210807875;
    verifyExistenceOfPeriodicOrbit(pm,center+X,2);

// period 3 orbit
    center[0] = -6.233158628537965;
    center[1] = 0.03064011165815;
    verifyExistenceOfPeriodicOrbit(pm,center+X,3);
