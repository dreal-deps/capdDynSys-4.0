...
public:
  ...
  bool getValue(const std::string & key, T & d) {
    ...
    ss.get();  // needed for some types that stop reading after some marker e.g. {232,3223} and do not set eof flag
    if(!ss.eof()){
      capd::slog << "\n[ Problem when reading parameter '" << key << "' from config file]" << std::endl;
      capd::slog << " value was : [" << this->operator [](key) << "]\n we read : [" << result << "]";
      capd::slog << "\n We do not change variable value which is : [" << d << "]" << std::endl;
      return false;
    }
    d = result;
    capd::sbug << "parameter read : " << key << " -> ["<< d << "]" << std::endl;
    return true;
  }
  ...
  bool getValues(const std::string & key, T begin, T end) {
    ...
    while(actual != end){
      if(ss.eof()){
        capd::slog << "\n[ Problem when reading table '" << key << "' from config file]" << std::endl;
        return false;
      }
      ss >> *actual;
      ++actual;
